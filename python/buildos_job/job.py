...
class Job:
    ...
    def _run(self, cmd: list[str], *, cwd: Optional[_Path] = None, env=None):
        ...
        start = _time.time(); env = env or _os.environ.copy()
        self.process = _sp.Popen(cmd, stdout=_sp.PIPE, stderr=_sp.STDOUT,
        ...
        try:
            while True:
                if self._stop_event.is_set():
                    self._terminate_pg(); raise JobCancelled()
                if _time.time() - start > self.timeout:
                    self._log(_log.ERROR, "Timeout exceeded"); self._terminate_pg()
    ...
    def run(self) -> bool:
        ...
        env = _os.environ.copy(); env["HOME"] = str(self.home_dir)
        try:
            ...
            if result_dir.is_dir():
                self._maybe_abort()
                try:
                    zip_path = _shutil.make_archive(str(self.clone_dir/self.id), 'zip', str(result_dir))
                    with open(zip_path,'rb') as fh:
                        db.update_task_content(self.id, fh.read())
                    self._log(_log.INFO, f"Archived results -> {zip_path}")
                except Exception as exc:
                    self._log(_log.ERROR, f"Archive failed: {exc}"); raise
            else:
                self._log(_log.INFO, "No .result directory – nothing to archive")

            self.status = Job.STATUS_FINISHED; return True
    ...
    def kill(self):
        self._stop_event.set(); self._terminate_pg()
